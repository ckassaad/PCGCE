...
def run_on_data(i, method, files_input_name, verbose):
    ...
    data = pd.read_csv('./data/fMRI_processed_by_Nauta/returns/selected/' + file_input_name, delimiter=',', index_col=False, header=0)
    ...
    three_col_format_ground_truth = np.loadtxt('./data/fMRI_processed_by_Nauta/ground_truth/' + file_ground_truth_name + '.csv',
                                               delimiter=',')
    ...
    if method == "GrangerPW":
        ...
        model = cd.GrangerLasso(nodes, nlags=5)
        model.infer_from_data(data)
    elif method == "TCDF":
        ...
        model = cd.PCTCE(nodes, sig_level=0.05, nlags=5)
        model.infer_from_data(data)
    elif method == "NBCB_pw":
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        method = "GrangerLasso"  # GrangerPW, GrangerMV, TCDF, PCMCICMIknn, PCMCIParCorr, PCTMI, tsFCI, FCITMI VarLiNGAM, TiMINO, Dynotears
        num_processor = 1
        verbose = True
        print('Default Argument List:', str(method), num_processor)

    # path_input = './data/fMRI_processed_by_Nauta/returns/small_datasets/'
    path_input = './data/fMRI_processed_by_Nauta/returns/selected/'
