...
from baselines.scripts_python.pcmciplus import pcmciplus
...
from baselines.scripts_python.granger_lasso import granger_lasso
from baselines.scripts_python.ocse import ocse

from baselines.scripts_python.pctmi import pctmi, tpctmi, tpcmcitmi
from baselines.scripts_python.fcitmi import fcitmi
from baselines.scripts_python.tskiko import tskiko
from baselines.scripts_python.pctce import pctce
from baselines.scripts_python.fcitce import fcitce
...
class GrangerLasso(GraphicalModel):
    def __init__(self, nodes, sig_level=0.05, nlags=5):
        GraphicalModel.__init__(self, nodes)
        self.sig_level = sig_level
        self.nlags = nlags
    ...
    def infer_from_data(self, data):
        data.columns = list(self.ghat.nodes)
        g_df = granger_lasso(data, maxlag=self.nlags, sig_level=self.sig_level)
        self._dataframe_to_graph(g_df)
...
class PCMCIplus(TemporalGraphicalModel):
    def __init__(self, nodes, sig_level=0.05, nlags=5, cond_ind_test="CMIknn"):
        TemporalGraphicalModel.__init__(self, nodes)
        self.sig_level = sig_level
        self.nlags = nlags
    ...
    def infer_from_data(self, data):
        data.columns = list(self.tghat.nodes)
        tg_dict = pcmciplus(data, tau_max=self.nlags, cond_ind_test=self.cond_ind_test, alpha=self.sig_level)
        self._dict_to_tgraph(tg_dict)
        self._tgraph_to_graph()
...
class TPCMCITMI(TemporalGraphicalModel):
    def __init__(self, nodes, sig_level=0.05, nlags=5):
        TemporalGraphicalModel.__init__(self, nodes)
        self.sig_level = sig_level
        self.nlags = nlags
    ...
    def infer_from_data(self, data):
        data.columns = list(self.ghat.nodes)
        tg_dict = tpcmcitmi(data, sig_level=self.sig_level, nlags=self.nlags, verbose=True)
        self._dict_to_tgraph(tg_dict)
        self._tgraph_to_graph()


class PCTCE(GraphicalModel):
    def __init__(self, nodes, sig_level=0.05, nlags=5):
        GraphicalModel.__init__(self, nodes)
        self.sig_level = sig_level
        self.nlags = nlags
    ...
    def infer_from_data(self, data):
        data.columns = list(self.ghat.nodes)
        self.ghat, self.oghat, self.sghat = pctce(data, sig_level=self.sig_level, nlags=self.nlags, verbose=True)

class FCITCE(GraphicalModel):
    def __init__(self, nodes, sig_level=0.05, nlags=5):
        GraphicalModel.__init__(self, nodes)
        self.sig_level = sig_level
        self.nlags = nlags
    ...
    def infer_from_data(self, data):
        data.columns = list(self.ghat.nodes)
        self.ghat, self.oghat, self.sghat = fcitce(data, sig_level=self.sig_level, nlags=self.nlags, verbose=True)
...
if __name__ == "__main__":
    ...
    TGtrue = nx.DiGraph()
    TGtrue.add_nodes_from(["V1", "V2", "V3", "v4"])
    TGtrue.add_edges_from([#("V1", "V1"), ("V2", "V2"), ("V3", "V3"), ("V4", "V4"),
                           ("V1", "V2"), ("V1", "V3"), ("V1", "V4"),
                           ("V2", "V1"), ("V2", "V3"), ("V2", "V4"),
                           ("V3", "V1"), ("V3", "V2"), ("V3", "V4"),
                           ("V4", "V1"), ("V4", "V2"), ("V4", "V3")])
    ...
    TGhat = nx.DiGraph()
    TGhat.add_nodes_from(["V1", "V2", "V3", "v4"])
    TGhat.add_edges_from([#("V1", "V1"), ("V2", "V2"), ("V3", "V3"), ("V4", "V4"),
                           ("V1", "V2"), ("V1", "V3"),
                           ("V2", "V4"),
                           ("V3", "V4"),
                           ("V4", "V2"), ("V4", "V3")])

    cl = GraphicalModel(["V1", "V2", "V3", "v4"])
    cl.ghat = TGhat

    print(cl._f1(TGtrue, method="all_oriented"))
    # print(cl._f1(TGtrue, method="other_oriented"))
